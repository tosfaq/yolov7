...
def plot_one_box(x, img, color=None, label=None, line_thickness=3, font_color=None):
    # Plots one bounding box on image img
    tl = line_thickness or round(0.002 * (img.shape[0] + img.shape[1]) / 2) + 1  # line/font thickness
    color = color or [random.randint(0, 255) for _ in range(3)]
    font_color = font_color or [255, 255, 255]  # white by default
    ...
    if label:
        ...
        cv2.putText(img, label, (c1[0], c1[1] - 2), 0, tl / 3, font_color, thickness=tf, lineType=cv2.LINE_AA)
