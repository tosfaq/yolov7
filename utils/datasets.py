...
def img2label_paths(img_paths):
    sa, sb = os.sep + 'images' + os.sep, os.sep + 'labels' + os.sep  # /images/, /labels/ substrings
    return [img_path.replace(sa, sb, 1).split('.')[0] + ".txt" for img_path in img_paths]



class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(self, path, img_size=640, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
                 cache_images=False, single_cls=False, stride=32, pad=0.0, prefix='', window_level=0, window_width=4500, cache_folder='./'):
        self.img_size = img_size
        ...
        cache_path = Path(cache_folder) / os.sep.join(str(cache_path).split(os.sep)[-3:]) # store in local folder
...
def load_image(self, index, window_level=0, window_width=4500):
    ...
    if img is None:  # not cached
        ...
        if r != 1:  # always resize down, only resize up if training with augmentation
            ...
            interp = cv2.INTER_AREA if r < 1 and not self.augment else cv2.INTER_NEAREST
